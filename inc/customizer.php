<?php
/**
 * twentytwelve Theme Customizer
 *
 * @package ThemeGrill
 * @subpackage twentytwelve
 * @since twentytwelve 1.0
 */
...
function twentytwelve_customize_preview_js() {
	wp_enqueue_script( 'twentytwelve_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20130508', true );
}
add_action( 'customize_preview_init', 'twentytwelve_customize_preview_js' );

/*****************************************************************************************/

function twentytwelve_register_theme_customizer( $wp_customize ) {
	// remove control
	$wp_customize->remove_control('blogdescription');
	 
	// rename existing section
	$wp_customize->add_section( 'title_tagline' , array(
		'title' => __('Site Title', 'twentytwelve' ),
		'priority' => 20
	) );

	class twentytwelve_ADDITIONAL_Control extends WP_Customize_Control {
	...
	$wp_customize->add_setting(
		'twentytwelve_color_scheme',
			array(
				'default'     	=> '#632E9B',
				'sanitize_callback' => 'twentytwelve_sanitize_hex_color',
				'sanitize_js_callback' => 'twentytwelve_sanitize_escaping'	
			)
	);
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'color_scheme',
			array(
				'label'      	=> __( 'Primary Color', 'twentytwelve' ),
				'section'    	=> 'colors',
				'settings'   	=> 'twentytwelve_color_scheme'
			)
		)
	);
/*custom-css*/
	$wp_customize->add_section(
		'twentytwelve_custom_css_section',
		array(
			'title'     => __( 'Custom CSS', 'twentytwelve' ),
			'priority'  => 200
		)
	);

	$wp_customize->add_setting(
		'twentytwelve_custom_css',
		array(
		'default'    =>  '',
		'sanitize_callback' => 'twentytwelve_sanitize_custom_css',
		'sanitize_js_callback' => 'twentytwelve_sanitize_escaping'
		)
	);

	$wp_customize->add_control(
		new twentytwelve_ADDITIONAL_Control (
			$wp_customize,
			'twentytwelve_custom_css',			
			array(				
				'label'    	=> __( 'Add your custom css here and design live! (for advanced users)' , 'twentytwelve' ),
				'section'   => 'twentytwelve_custom_css_section',
				'settings'  => 'twentytwelve_custom_css'
			)
		)
	);
/*end custom-css*/
	$wp_customize->add_section(
		'twentytwelve_featured_section',
		array(
			'title'     => __( 'Front Page Featured Section', 'twentytwelve' ),
			'priority'  => 220
		)
	);
	...
	$wp_customize->add_setting(
		'page-setting-one',
		array(
			'sanitize_callback' => 'twentytwelve_sanitize_integer'
		)
	);
	$wp_customize->add_setting(
		'page-setting-two',
	...
	$wp_customize->add_control(
		'page-setting-one',
			array(
			'type' => 'dropdown-pages',
			'label' => __( 'First featured page', 'twentytwelve' ),
			'section' => 'twentytwelve_featured_section',
		)
	);
	$wp_customize->add_control(
		'page-setting-two',
			array(
			'type' => 'dropdown-pages',
			'label' => __( 'Second featured page', 'twentytwelve' ),
			'section' => 'twentytwelve_featured_section',
		)
	);
	$wp_customize->add_control(
		'page-setting-three',
			array(
			'type' => 'dropdown-pages',
			'label' => __( 'Third featured page', 'twentytwelve' ),
			'section' => 'twentytwelve_featured_section',
		)
	);
        
    /*color-scheme*/    
        $wp_customize->add_section( 'twentytwelve_color_scheme_settings', array(
		'title'       => __( 'Color Scheme', 'twentytwelve' ),
		'priority'    => 30,
		'description' => 'Select your color scheme',
	) );

	$wp_customize->add_setting( 'twentytwelve_color_scheme', array(
		'default'        => 'red',
	) );

	$wp_customize->add_control( 'twentytwelve_color_scheme', array(
		'label'   => 'Choose your color scheme',
		'section' => 'twentytwelve_color_scheme_settings',
		'default'        => 'red',
		'type'       => 'radio',
		'choices'    => array(
			__( 'blue', 'locale' ) => 'blue',
...
/*end custom layout*/
	...
	function twentytwelve_sanitize_hex_color( $color ) {
	...
	function twentytwelve_sanitize_custom_css( $input) {
		$input = wp_kses_stripslashes( $input);
		return $input;
	}	

	function twentytwelve_sanitize_integer( $input ) {
    	if( is_numeric( $input ) ) {
        return intval( $input );
   	}
	}

	function twentytwelve_sanitize_escaping( $input) {
		$input = esc_attr( $input);
		return $input;
	}

}
add_action( 'customize_register', 'twentytwelve_register_theme_customizer' );


function twentytwelve_customizer_css() {
	$primary_color =  get_theme_mod( 'twentytwelve_color_scheme' );
	...
	?>
	<style type="text/css"><?php echo trim( get_theme_mod( 'twentytwelve_custom_css' ) ); ?></style>
	<?php
}
add_action( 'wp_head', 'twentytwelve_customizer_css' );
?>
